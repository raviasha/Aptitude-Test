...
if not defined KSAT_UPDATE_SIGNING_PUBLIC_KEY (
  echo KSAT_UPDATE_SIGNING_PUBLIC_KEY is required for a production release.
  exit /b 1
)
...
if defined ISCC_EXE (
  "%PYTHON_EXE%" scripts\windows_release.py all --root "%CD%" --python "%PYTHON_EXE%" --iscc "%ISCC_EXE%" --signing-pfx "%KSAT_SIGNING_PFX%" --signing-publisher "%KSAT_SIGNING_PUBLISHER%" --timestamp-url "%KSAT_SIGNING_TIMESTAMP_URL%" --update-signing-public-key "%KSAT_UPDATE_SIGNING_PUBLIC_KEY%" || exit /b 1
) else (
  "%PYTHON_EXE%" scripts\windows_release.py all --root "%CD%" --python "%PYTHON_EXE%" --signing-pfx "%KSAT_SIGNING_PFX%" --signing-publisher "%KSAT_SIGNING_PUBLISHER%" --timestamp-url "%KSAT_SIGNING_TIMESTAMP_URL%" --update-signing-public-key "%KSAT_UPDATE_SIGNING_PUBLIC_KEY%" || exit /b 1
)
