...
1. Keep `KSATClientSetup-2.1.0.exe` and the release `SHA256SUMS.txt` in the same folder.
...
The script checks the local SHA-256 and Authenticode signature, DNS, 64-bit Windows, free space, remote access, installation result, and the client `/api/build` health response. It removes the temporary task and installer after every attempt. Reports contain hostnames, stages, version, timestamps, and stable error codes; they do not contain credentials.
