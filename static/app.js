...
async function tests() {
  ...
  const facultyTimer = setInterval(() => document.querySelectorAll('[data-faculty-timer]').forEach(node => { const seconds = Math.max(0, Math.ceil((Number(node.dataset.deadline) - Date.now()) / 1000)); node.textContent = formatTime(seconds); }), 1000);
