...
async function studentDashboard() {
  ...
  layout(`Good to see you, <em>${esc(data.student.name.split(' ')[0])}.</em>`, 'Build a focused practice set or continue a Faculty assessment.', `
    <section class="hero"><div><p class="eyebrow">${data.launched ? 'Faculty-launched assessment' : 'Available assessments'}</p><h2>${test ? esc(test.test_name) : 'No Faculty assessment is available'}</h2><p>${test ? (data.launched ? 'Only this launched assessment can be attempted now.' : 'Choose an assessment below when you are ready.') : 'You can still build your own practice set below.'}</p></div>${active ? `<button class="primary" data-resume="${active.attempt_id}">Resume ${active.mode === 'student_practice' ? 'practice' : 'assessment'} →</button>` : ''}</section>
