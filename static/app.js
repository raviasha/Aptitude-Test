...
async function api(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    const message = typeof detail === 'string' ? detail : (detail && typeof detail.message === 'string' ? detail.message : null);
    throw new Error(message || `The server could not complete this request (HTTP ${response.status}).`);
  }
...
function layout(title, subtitle, content, nav = '') {
  ...
  app.innerHTML = faculty ? `<div class="faculty-shell">
    ...
    <div class="faculty-body">
      <header class="faculty-topbar"><span>Faculty portal <span class="breadcrumb-divider">/</span> <strong>${({overview:'Overview',banks:'Question banks',tests:'Tests',students:'Students'})[nav.match(/aria-current="page" data-nav="(\w+)"/)?.[1]] || 'Workspace'}</strong></span><div class="faculty-account"><span class="avatar" aria-hidden="true">${esc((state.user.name || 'F').slice(0,1).toUpperCase())}</span><span>${esc(state.user.name || 'Faculty')}<small>Prof R Ravi Shankar</small></span><button class="secondary small" data-logout>Sign out</button></div></header>
...
function loginScreen() {
    app.innerHTML = `<div class="login campus-login"><section class="login-intro" aria-hidden="true"></section><section class="login-card campus-login-card"><a class="brand login-brand"><span>A</span>AIML - KSAT</a><p class="eyebrow">Welcome back</p><h2>Sign in</h2><div class="role"><button type="button" class="chosen" data-role="student" aria-pressed="true">Student</button><button type="button" data-role="admin" aria-pressed="false">Faculty</button></div><form id="login-form"><label>Department<select id="department" required>${departments.map(department => `<option value="${esc(department)}" ${department === 'AIML' ? 'selected' : ''}>${esc(department)}</option>`).join('')}</select></label><label id="login-id-label">Student ID / USN<input id="identifier" required placeholder="1KS23AI042" autocomplete="username" /></label><label>Password<input id="password" type="password" required placeholder="Enter password" autocomplete="current-password" /></label><button class="primary">Sign in →</button></form><p class="demo" id="demo">Student demo: <code>1KS23AI042</code> / <code>student123</code></p><button class="secondary link" id="register-link">New student? Register here</button><small>Securely hosted inside the college network. · Build ${BUILD_VERSION}</small><small class="rights-notice">Rights Reserved: AIML Department, KSIT</small></section></div>`;
  let role = 'student';
  document.querySelectorAll('[data-role]').forEach(button => button.addEventListener('click', () => {
    role = button.dataset.role; document.querySelectorAll('[data-role]').forEach(item => { const chosen = item === button; item.classList.toggle('chosen', chosen); item.setAttribute('aria-pressed', String(chosen)); });
    ...
    demo.hidden = role !== 'student';
    demo.innerHTML = role === 'student' ? 'Student demo: <code>1KS23AI042</code> / <code>student123</code>' : '';
  }));
...
function renderAttempt() {
  ...
  const feedback = attempt.feedback_allowed && q.feedback ? `<article class="card feedback"><h3>${q.feedback.correct ? 'Correct' : 'Not quite'}</h3><p><strong>Correct answer:</strong> ${q.feedback.correct_answer}</p>${q.feedback.solution_steps?.length ? `<h4>Solution steps</h4>${solutionMedia.length ? `<ol class="sr-only">${q.feedback.solution_steps.map(step => `<li>${esc(step)}</li>`).join('')}</ol>` : `<ol>${q.feedback.solution_steps.map(step => `<li>${esc(step)}</li>`).join('')}</ol>`}` : ''}${solutionMedia.map((_, index) => mediaMarkup(q.feedback?.display_media?.solution?.[index], 'solution-media')).join('')}</article>` : '';
  const textQuestion = q.question_html ? `<div class="visual-question">${q.question_html}</div>` : `<h1>${esc(q.question_text)}</h1>`;
...
async function tests() {
  ...
  manageAttempts.addEventListener('click', async () => {
    const choices = data.tests.map(test => `${test.test_id}: ${test.test_name}`).join('\n');
    const testId = prompt(`Assessment ID to inspect:\n${choices}`); if (testId === null) return;
    try {
      const detail = await api(`/api/admin/tests/${encodeURIComponent(testId)}/distributed`);
      if (!detail.attempts.length) { alert('This assessment has no attempts.'); return; }
      alert(detail.attempts.map(item => `${item.attempt_id} · ${item.student_id} · ${item.status} · deadline ${item.deadline || '—'} · score ${item.submitted_at ? item.score : '—'} · violations ${item.violation_count}`).join('\n'));
      const attemptId = prompt('Attempt ID to inspect or change:'); if (attemptId === null) return;
      const attempt = detail.attempts.find(item => item.attempt_id === attemptId); if (!attempt) throw new Error('Choose an attempt ID shown in the status list.');
      const action = prompt('Action: order, extend, or void'); if (action === null) return;
      if (action.trim().toLowerCase() === 'order') { const order = await api(`/api/admin/attempts/${encodeURIComponent(attemptId)}/order`); alert(`Exact question order: ${order.question_order.join(', ')}`); return; }
      const reason = prompt(`Reason for ${action.trim().toLowerCase()}:`); if (reason === null) return;
      if (action.trim().toLowerCase() === 'extend') { const minutes = Number(prompt('Additional minutes:', '5')); await api(`/api/admin/attempts/${encodeURIComponent(attemptId)}/extend`, {method:'POST',body:{minutes,reason}}); notify('Attempt timer extended.'); tests(); return; }
      if (action.trim().toLowerCase() === 'void') { if (!confirm('Void this attempt while preserving all evidence?')) return; const authorizeRetake=confirm('Authorize one replacement attempt?'); const confirmSubmitted=attempt.status==='submitted' ? confirm('This submission is accepted. Confirm that its result should be voided but preserved.') : false; await api(`/api/admin/attempts/${encodeURIComponent(attemptId)}/void`, {method:'POST',body:{reason,authorize_retake:authorizeRetake,confirm_submitted:confirmSubmitted}}); notify('Attempt voided and audit evidence preserved.'); tests(); return; }
      throw new Error('Choose order, extend, or void.');
    } catch(error) { notify(error.message,true); }
  });
  ...
  document.querySelectorAll('[data-extend-test]').forEach(button => button.addEventListener('click', async () => { const reason=prompt('Reason for the audited five-minute extension:'); if (reason===null) return; try { await api(`/api/admin/tests/${button.dataset.extendTest}/extend`, {method:'POST',body:{minutes:5,reason}}); notify('Active timers and future starts were extended by five minutes.'); tests(); } catch(error) { notify(error.message,true); } }));
