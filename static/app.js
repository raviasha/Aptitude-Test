...
async function questionBanks() {
  ...
  document.querySelectorAll('[data-delete-bank]').forEach(button => button.addEventListener('click', async () => { if (!confirm(`Delete “${button.dataset.bankName}” and its questions/visuals? This cannot be undone.`)) return; try { const result = await api(`/api/admin/question-banks/${button.dataset.deleteBank}`, {method:'DELETE'}); notify(`${result.bank_name} deleted.`); questionBanks(); } catch(error) { notify(error.message,true); } }));
}
