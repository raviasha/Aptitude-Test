...
function loginScreen() {
  app.innerHTML = `<div class="login campus-login"><section class="login-intro" aria-hidden="true"></section><section class="login-card campus-login-card"><a class="brand login-brand"><span>A</span>AIML-<i>KSAT</i></a><p class="eyebrow">Welcome back</p><h2>Sign in</h2><div class="role"><button type="button" class="chosen" data-role="student" aria-pressed="true">Student</button><button type="button" data-role="admin" aria-pressed="false">Faculty</button></div><form id="login-form"><label>Department<select id="department" required><option value="AIML" selected>AIML</option></select></label><label id="login-id-label">Student ID / USN<input id="identifier" required placeholder="1KS23AI042" autocomplete="username" /></label><label>Password<input id="password" type="password" required placeholder="Enter password" autocomplete="current-password" /></label><button class="primary">Sign in →</button></form><p class="demo" id="demo">Student demo: <code>1KS23AI042</code> / <code>student123</code></p><button class="secondary link" id="register-link">New student? Register here</button><small>Securely hosted inside the college network. · Build ${BUILD_VERSION}</small></section></div>`;
  let role = 'student';
