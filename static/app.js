...
async function studentDashboard() {
  ...
  layout(`Good to see you, <em>${esc(data.student.name.split(' ')[0])}.</em>`, 'Choose an assessment, work at your pace, and see exactly where you can improve.', `
    <section class="hero"><div><p class="eyebrow">Available assessment</p><h2>${test ? esc(test.test_name) : 'No assessment is available'}</h2><p>${test ? 'Questions are selected and ordered randomly. Your answer saves immediately after every choice.' : 'Ask faculty to publish a test for your class.'}</p></div>${active ? `<button class="primary" data-resume="${active.attempt_id}">Resume assessment →</button>` : test ? `<button class="primary" data-start="${test.test_id}">Start assessment →</button>` : ''}</section>
...
function renderAttempt() {
  ...
  app.innerHTML = `<header class="top"><a class="brand"><span>A</span>Aptitude <i>Lab</i></a><div class="save">✓ Answer saved automatically</div><button class="ghost" data-exit>Save and exit</button></header><main class="assessment"><aside><p class="eyebrow">Progress</p><strong>${answered} <small>/ ${attempt.questions.length}</small></strong><p>${answered} answered</p><div class="numbers">${attempt.questions.map((item,index) => `<button class="${index===state.questionIndex?'current':''} ${item.selected_answer?'done':''}" data-index="${index}">${index+1}</button>`).join('')}</div></aside><section class="question"><div class="question-meta"><span>${esc(short(q.category))}</span><span>Question ${state.questionIndex+1} of ${attempt.questions.length}</span></div>${q.question_html ? `<div class="visual-question">${q.question_html}</div>` : `<h1>${esc(q.question_text)}</h1>`}<div class="answers">${Object.entries(q.options).map(([key,value]) => `<button class="${q.selected_answer===key?'selected':''}" data-answer="${key}"><i>${key}</i>${esc(value)}<b>${q.selected_answer===key?'✓':''}</b></button>`).join('')}</div><footer><button class="secondary" data-prev ${state.questionIndex===0?'disabled':''}>← Previous</button>${state.questionIndex===attempt.questions.length-1 ? '<button class="primary" data-submit>Review & submit →</button>' : '<button class="primary" data-next>Next question →</button>'}</footer></section></main>`;
...
async function tests() { const data = await api('/api/admin/tests'); layout('Create a <em>test.</em>', 'Select one question bank. The server validates category availability before publishing.', `<section class="grid two"><article class="card"><p class="eyebrow">New assessment</p><h2>Question composition</h2><form id="test-form"><label>Test name<input name="test_name" required placeholder="Placement Readiness · Set 02" /></label><label>Question bank<select name="bank_id" required><option value="">Choose a bank…</option>${data.banks.map(bank => `<option value="${bank.bank_id}">${esc(bank.bank_name)} · ${bank.question_count} active questions</option>`).join('')}</select></label><div class="composition">${categories.map(category => `<label><span>${esc(category)}</span><input type="number" name="${esc(category)}" min="0" max="100" value="6" /></label>`).join('')}</div><button class="primary">Create test →</button></form></article><article class="card"><p class="eyebrow">Current tests</p><h2>Test library</h2><table><thead><tr><th>Name</th><th>Bank</th><th>Questions</th></tr></thead><tbody>${data.tests.map(test => `<tr><td>${esc(test.test_name)}</td><td>${esc(test.bank_name || '—')}</td><td>${Object.values(test.composition).reduce((sum,value)=>sum+value,0)}</td></tr>`).join('')}</tbody></table></article></section>`, adminNav('tests')); document.querySelector('#test-form').addEventListener('submit', async event => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); const composition = {}; categories.forEach(category => composition[category] = Number(values[category] || 0)); try { await api('/api/admin/tests',{method:'POST',body:{test_name:values.test_name,bank_id:Number(values.bank_id),composition}}); notify('Test created.'); tests(); } catch(error) { notify(error.message,true); } }); }

async function students() { const data = await api('/api/admin/students'); layout('Manage <em>students.</em>', 'Student identities and passwords are stored centrally on the server.', `<section class="grid two"><article class="card"><p class="eyebrow">Add student</p><h2>Lab roster</h2><form id="student-form"><label>Student name<input name="name" required /></label><label>Student ID / USN<input name="student_id" required /></label><div class="split"><label>Class<input name="student_class" value="AI & DS" /></label><label>Section<input name="section" value="A" /></label></div><label>Temporary password<input name="password" value="student123" required /></label><button class="primary">Add student →</button></form></article><article class="card"><p class="eyebrow">Enrolled students</p><h2>${data.students.length} students</h2><table><thead><tr><th>Name</th><th>USN</th><th>Class</th></tr></thead><tbody>${data.students.map(student => `<tr><td>${esc(student.name)}</td><td><code>${esc(student.student_id)}</code></td><td>${esc(student.class)}-${esc(student.section)}</td></tr>`).join('')}</tbody></table></article></section>`, adminNav('students')); document.querySelector('#student-form').addEventListener('submit', async event => { event.preventDefault(); try { await api('/api/admin/students',{method:'POST',body:Object.fromEntries(new FormData(event.currentTarget))}); notify('Student added.'); students(); } catch(error) { notify(error.message,true); } }); }
