...
async function tests() {
  ...
  document.querySelector('[data-rotate-enrollment]')?.addEventListener('click', async () => { if (!confirm('Rotate the device enrollment code? The previous code stops working immediately.')) return; const reason=prompt('Reason for rotating the enrollment code:'); if (reason===null) return; try { const result=await api('/api/admin/devices/enrollment-code/rotate',{method:'POST',body:{reason}}); alert(`New enrollment code (shown once): ${result.enrollment_code}`); } catch(error) { notify(error.message,true); } });
